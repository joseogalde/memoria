...
legend('simulation variance',...
    strcat('model R^2 = ',num2str(godness.rsquare)));
textX = 10;
textY = 0.02;
...
legend('simulation variance',...
    strcat('model R^2 = ',num2str(godness.rsquare)), 'experimental variance');
textX = 10;
textY = 0.02;
text(textX,textY,strcat('\sigma^{2} = ',fitEquation{eqNum}), ...
    'Color',modelColor,'FontSize',20, 'HorizontalAlignment','left');
...
xlabel('Hz');
ylabel('%');
title('Error between experimental variance and expected variance');
